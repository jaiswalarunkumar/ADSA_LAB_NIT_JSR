...
void radixSort(int arr[], int n) {
    int m=getMax(arr,n);
    for(int exp=1;m/exp>0;exp*=10) countSortRadix(arr,n,exp);
}
...
// (x) Bucket Sort (for numbers 0–1 floating values mapped to buckets)
...
// (xi) Radix Exchange Sort (Binary QuickSort idea)
void radixExchangeSort(int arr[], int left, int right, int bit) {
    if (left >= right || bit < 0) return;
    int i=left, j=right;
    while (i<=j) {
        while(i<=right && !(arr[i] & (1<<bit))) i++;
        while(j>=left && (arr[j] & (1<<bit))) j--;
...
// (xii) Address Calculation Sort (Hash sort idea)
void addressCalculationSort(int arr[], int n) {
    int max=getMax(arr,n);
    int *pos=(int*)calloc(max+1,sizeof(int));
    for(int i=0;i<n;i++) pos[arr[i]]=arr[i];
    int idx=0;
    for(int i=0;i<=max;i++)
        if(pos[i]!=0) arr[idx++]=pos[i];
    free(pos);
}

/* ---------------- Main ---------------- */
int main() {
    int arr[] = {29, 10, 14, 37, 13, 5, 78, 25};
